      ...
      <div className="mx-auto flex h-[60px] max-w-[1280px] items-center justify-between gap-4 px-4 sm:px-6">
        <div className="flex min-w-0 items-center gap-3">
          <div className="flex items-center gap-2.5 py-1 pr-2 pl-1">
            <div className="flex h-8 w-8 items-center justify-center rounded-lg text-[11px] font-semibold tracking-wide text-white" style={{ backgroundColor: currentBrand?.theme_accent_color || '#5B4FE0' }}>
              {(currentBrand?.name || 'Aatmi').slice(0, 2).toUpperCase()}
            </div>
            <div className="hidden text-left sm:block">
              <div className="font-display text-[13px] font-semibold leading-tight text-[var(--color-text-primary)]">
                <span className="block max-w-[180px] truncate">{currentBrand?.name || 'Platform admin'}</span>
              </div>
              <div className="font-mono text-[10px] tracking-wide text-[var(--color-text-tertiary)]">{currentBrand?.slug || 'admin'}</div>
            </div>
          </div>
        </div>
